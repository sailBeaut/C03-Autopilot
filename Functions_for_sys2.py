...
def accuracy_plot_elev(accuracy_dof1_array, accuracy_dof2_array):
    ...
    avg_acc1 = sum(accuracy_dof1_array) / len(accuracy_dof1_array)
    avg_acc2 = sum(accuracy_dof2_array) / len(accuracy_dof2_array)
    avg_acc1_wo5 = (sum(accuracy_dof1_array[:-1])-accuracy_dof1_array[1]) / len(accuracy_dof1_array[:-2])
    avg_acc2_wo5 = (sum(accuracy_dof2_array[:-1])-accuracy_dof2_array[1]) / len(accuracy_dof2_array[:-2])
    avg_acc1_wo4n5 = (sum(accuracy_dof1_array[:-1])-accuracy_dof1_array[0]) / len(accuracy_dof1_array[:-2])
    avg_acc2_wo4n5 = (sum(accuracy_dof2_array[:-1])-accuracy_dof2_array[0]) / len(accuracy_dof2_array[:-2]) 
    plt.figure(figsize=(10, 5))
    plt.plot(accuracy_dof1_array, label="DOF1", color="red")
    plt.plot(accuracy_dof2_array, label="DOF2", color="blue")
    plt.axhline(y=avg_acc1, color='r', linestyle='--', label=f"Average DOF1: {avg_acc1:.2f}%")
    plt.axhline(y=avg_acc2, color='b', linestyle='--', label=f"Average DOF2: {avg_acc2:.2f}%")
    plt.axhline(y=avg_acc1_wo5, color='green', linestyle=':', label=f"Average DOF1 (no run5): {avg_acc1_wo5:.2f}%")
    plt.axhline(y=avg_acc2_wo5, color='orange', linestyle=':', label=f"Average DOF2 (no run5): {avg_acc2_wo5:.2f}%")
    plt.axhline(y=avg_acc1_wo4n5, color='purple', linestyle='-.', label=f"Average DOF1 (no run4 and run5): {avg_acc1_wo4n5:.2f}%")
    plt.axhline(y=avg_acc2_wo4n5, color='yellow', linestyle='-.', label=f"Average DOF2 (no run4 and run5): {avg_acc2_wo4n5:.2f}%")
    plt.xlabel("Run")
    plt.ylabel("Accuracy (%)")
