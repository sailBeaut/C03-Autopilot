...
def model2_aileron(run, k_g, k1_numvalue, k2_numvalue, c1_numvalue, c2_numvalue, a_velo, extragraphs, showmainplots, printeigenvalues):
    ...
    absolute_error1 = np.abs(0.5*Y_sol[:, 0] - DeltaDrumAil)
    ...
    if showmainplots == True:
        ...
        plt.subplot(2, 4, 5)
        plt.plot(t_values, 0.5*Y_sol[:, 0], label="x1 (DOF 1)")
        ...
        plt.subplot(2, 4, 6)
        percentage_error_dof1 = np.abs((0.5*Y_sol[:, 0] - DeltaDrumAil) / DeltaDrumAil) * 100
    ...
    if extragraphs == True:
        plt.plot(t_values, 0.5*Y_sol[:, 0]*(180*np.pi), label="x1 (DOF 1)")
        ...
        plt.plot(t_values, (-Y_sol[:, 1]*(180*np.pi))/(0.5*Y_sol[:, 0]*(180*np.pi)), label="Linearity vs. cable slack")
        plt.xlabel("Time (s)")
        plt.ylabel("Ratio")
        plt.title("Linearity")
        plt.legend()
