...
k1_values = np.linspace(500, 5000, 5)   # Example range for stiffness k1
k2_values = np.linspace(1000, 10000, 5) # Example range for stiffness k2
c1_values = np.linspace(0.1, 5, 5)      # Example range for damping c1
c2_values = np.linspace(0.1, 5, 5)      # Example range for damping c2

#k1=5000.0, k2=10000.0, c1=5.0, c2=5.0
...
def rms_error(computed, real):
    """
    Computes the Root Mean Square (RMS) error between computed and real data.

    Parameters:
    computed (numpy.ndarray): Computed system response.
    real (numpy.ndarray): Real measured data.

    Returns:
    float: RMS error value.
    """
    # Ensure both arrays have the same length
    min_length = min(len(computed), len(real))
    computed = computed[:min_length]
    real = real[:min_length]

    # Compute RMS error
    return np.sqrt(np.mean((computed - real) ** 2))
...
error_ail = rms_error(Y_sol[:, 1], DeltaAil)  # Compare computed x2 (DOF 2) with DeltaAil
error_drum = rms_error(Y_sol[:, 0], DeltaDrumAil)  # Compare computed x1 (DOF 1) with DeltaDrumAil

print(f"RMS Error for Aileron: {error_ail}")
print(f"RMS Error for Drum Aileron: {error_drum}")

for k1_try, k2_try, c1_try, c2_try in itertools.product(k1_values, k2_values, c1_values, c2_values):
    ...
    # Compute errors
    error_ail = rms_error(Y_sol[:, 1], DeltaAil)
    error_drum = rms_error(Y_sol[:, 0], DeltaDrumAil)

    total_error = error_ail + error_drum  # Aggregate error

    # Check if this set is the best so far
    if total_error < lowest_error:
        lowest_error = total_error
        best_params = (k1_try, k2_try, c1_try, c2_try)

# Print best parameters and corresponding error
print(f"Best Parameters: k1={best_params[0]}, k2={best_params[1]}, c1={best_params[2]}, c2={best_params[3]}")
print(f"Lowest RMS Error: {lowest_error}")
